...
import com.a1101studio.mobile_helper_open.utils.HtmlHelper;
...
import static com.a1101studio.mobile_helper_open.R.string.ok;
...
public class MainActivity extends AppCompatActivity {
                ...
                public void onClick(DialogInterface dialog, int which) {
                    Observable.just(FileHelper.createOrGetFileDir(getApplicationContext())).map(file1 -> {
                        ...
                        public void onCompleted() {
                            new AlertDialog.Builder(MainActivity.this).setMessage("Данные успешно удалены!").setPositiveButton(ok,null).show();
                        }
    ...
    void createPDF() {//тут заодно перекидываем всё папку в нужное место
        if (ETCompanyName.getText().toString().trim().length() > 0
                && ETArea.getSelectedItem().toString().trim().length() > 0
                && ETElectricLine.getSelectedItem().toString().trim().length() > 0
                && ETNomination.getText().toString().trim().length() > 0
                && ETTypeOfInspection.getText().toString().trim().length() > 0
                && ETNumberStartInspectionSeat.getText().toString().trim().length() > 0
                && ETNumberEndInspectioSeat.getText().toString().trim().length() > 0
                && ETInspectorName.getText().toString().trim().length() > 0
                && Prinal.getText().toString().trim().length() > 0) {
